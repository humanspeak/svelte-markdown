...
| \`key=10\`<br>\`key>=2024-01-01\`                            | When using numeric operators for numbers or dates, the token will become a "keyword_numeric" or "keyword_date" token with the operator provided. See below (1) for supported date formats.                                           |
| \`key:10..20\`<br>\`key:2024-01-01 00:00..2024-01-15 12:00\` | Range queries allow you to specify a range of values. For ranges, use \`key:start..end\`. The result will be two "keyword_numeric" or "keyword_date" tokens. See below (1) for supported date formats.                                 |
